...
@app.get("/stock_analyzer",
         ...
         summary="分析单只股票",
         description="通过单只股票代码和市场类型分析股票，返回包括价格、评分、技术分析和AI分析等综合信息")
...
@app.get(
    ...
    summary="获取指定股票技术分析",
    description="获取指定股票的技术指标分析，包括MA趋势、RSI、MACD信号、布林带等技术指标"
)
...
@app.get(
    ...
    summary="获取指定股票AI分析",
    description="使用AI对指定股票进行深度分析，提供趋势、风险、目标价位等综合分析结果"
)
...
@app.get("/ali_search_web")
async def ali_mcp_search(query: str):
    """访问阿里云百炼的联网搜索MCP服务，当你无法回答用户的问题或者当问的是与股票具体指标信息无关时使用！"""
    try:
...
if __name__ == "__main__":
    ...
    # 启动服务
    uvicorn.run("mcp_server_integration_remote_mcp:app", host=host, port=port, reload=True)
